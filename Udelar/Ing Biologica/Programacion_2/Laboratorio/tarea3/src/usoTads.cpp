...
/*void recorrerArbolBinario(nat l, TBinario &b, TCadena cad){
  if( terminoDeRecorrerElArbol == NULL ){

  }else if( nivel == l ){
    insertarAlFinal( eseNodo, cad );
  }

  recorrerArbolBinario(l , b->izq  aca deberia ir una funcion que manipula la representacion del TBinario , cad);

}*/

TCadena nivelEnBinario(nat l, TBinario b){
  TCadena cad = crearCadena();
  /*nat maxNodosPorNivel = 2^(l-1);
  nat i = 1;
  while( i <= maxNodosPorNivel ){}
  insertarAlFinal( recorrerArbolBinario() ) ;
  cad = recorrerArbolBinario(l , b, cad);*/
